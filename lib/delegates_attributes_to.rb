...
module DelegatesAttributesTo
  ...
  included do
    alias_method_chain :assign_multiparameter_attributes, :delegation

    class_attribute :default_rejected_delegate_columns
    self.default_rejected_delegate_columns = DEFAULT_REJECTED_COLUMNS.dup

    class_attribute :delegated_attributes
    self.delegated_attributes = HashWithIndifferentAccess.new
  end
